...
class modSlideshowckHelper
{
    ...
    static function getArticle(&$item, $params)
    {
        ...
        $articles = JModelLegacy::getInstance('Articles', 'ContentModel', array(
            'ignore_request' => true
        ));
    ...
    public static function loadImagesFromFolder($directory)
    {
        ...
        if (! $files)
            return 'CK message : No files found in the directory : ' . $directory;
        ...
        foreach ($files as $file) {
            ...
            if (! in_array(strToLower($fileExt), $authorisedExt))
                continue;
    ...
    private static function getImageLabelsFromFolder($directory)
    {
        ...
        if (! empty(self::$folderLabels[$dirindex]))
            return;
    ...
    static function getItemsAutoloadarticlecategory(&$params)
    {
                        ...
                        case 'article':
                            if ($params->get('show_on_article_page', 1)) {
                                ...
                                if (! $catid) {
                                    ...
                                    $article = JModelLegacy::getInstance('Article', 'ContentModel', array(
                                        'ignore_request' => true
                                    ));
        ...
        if ($catids) {
            if ($params->get('show_child_category_articles', 0) && (int) $params->get('levels', 0) > 0) {
                ...
                $categories = JModelLegacy::getInstance('Categories', 'ContentModel', array(
                    'ignore_request' => true
                ));
        ...
        foreach ($items as &$item) {
            ...
            if ($article_image && (count($slideItems) < (int) $params->get('count', 0) || (int) $params->get('count', 0) == 0)) {
     ...
     * all open HTML tags.
     * The method will optionally not truncate an individual
    ...
    public static function truncate($text, $length = 0, $noSplit = true, $allowHtml = true)
    {
        ...
        if ($length > 0 && JString::strlen($text) > $length) {
            ...
            if ($allowHtml) {
                ...
                $openedTags = $result[1];
                $openedTags = array_diff($openedTags, array(
                    "img",
                    "hr",
                    "br"
                ));
     ...
     *            (if set true, returns the value separated by the separator character. Otherwise returns associative array)
