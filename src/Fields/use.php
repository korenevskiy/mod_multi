...
if ($param->work_type_require == 'and') :
    ...
    if ($param->menu_is) :
        ...
        if (in_array(0, $menu_assigment) || empty($menu_assigment)) {} elseif ($param->menu_is == 'only' && ! in_array($menu_id, $menu_assigment)) {
            return FALSE;
    ...
    if ($param->component_is) :
        ...
        if (empty($component_site)) {} elseif ($param->component_is == 'only' && ! in_array($component_id, $component_site)) {
            return FALSE;
    ...
    if ($param->view_is) :
        ...
        if (empty($views)) {} elseif ($param->view_is == 'only' && ! ModMultiHelper::inArray($view_site, $views)) {
            return FALSE;
    ...
    if ($param->date_is) :
        ...
        if (empty($date_start) || empty($date_stop)) {} else if ($date_stop < $now && $now < $date_start) {
            return FALSE;
    ...
    if ($param->date_is) :
        ...
        $date_stop = $param->date_stop;
        $now = JDate::getInstance()->format('Y-m-d');
        if (empty($date_start) || empty($date_stop)) {} else if ($date_start == $now || $now == $date_stop) {
            return TRUE;
