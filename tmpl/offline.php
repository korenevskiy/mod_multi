...
<img src="<?= $image ?>" alt="<?= $sitename ?>"
		style="width: auto; height: auto; margin: 0 -2000px; left: -5000px; position: relative; display: block;" />
	<div id="frame" class="outline"
		style="position: absolute; position: fixed; top: 20px; left: 20px; right: 20px; width: min-content; margin: auto; border-radius: 20px; box-shadow: 0 5px 20px #000; background-color: rgba(255, 255, 255, 0.9); display: table;">
		<img src="/modules/mod_multi/media/img/animated_girl.gif"
			alt="<?= $sitename ?>" style="border-radius: 10px; margin: 10px;" />
		<?php if ($image) : ?>
			<img src="<?= $image; ?>" alt="<?= $sitename ?>" />
		<?php endif; ?>
		<h1 style="border-radius: 10px; margin: 10px;">
			<?= $sitename ?> <a style="font-size: small;"
				href="mailto:<?= $mailfrom ?>" target="__blank"><?= $mailfrom ?> </a>
		</h1>
	...
	<form action="<?= '/administrator/index.php' ?>" method="post"
			...
			<fieldset class="input"
				style="border-radius: 10px; border: 1px solid gray; margin: 10px;">
				<p id="form-login-username"
					...
					<input name="username" id="username" type="text" class="inputbox"
						...
						alt="<?php echo $JText_('JGLOBAL_USERNAME'); ?>" size="18" />
				</p>
				<p id="form-login-password"
					style="display: flex; justify-content: space-between;">
					<label for="passwd" style="flex: 1;"><?php echo $JText_('JGLOBAL_PASSWORD'); ?></label>
					<input name="passwd" id="passwd" type="password" class="inputbox"
						size="18" style="border-radius: 5px;"
						alt="<?php echo $JText_('JGLOBAL_PASSWORD'); ?>" />
				</p>
		<?php if (count($twofactormethods) > 1) : ?>
			<p id="form-login-secretkey">
					<label for="secretkey"><?php echo $JText_('JGLOBAL_SECRETKEY'); ?></label>
					<input type="text" name="secretkey" class="inputbox" size="18"
						alt="<?php echo $JText_('JGLOBAL_SECRETKEY'); ?>" id="secretkey" />
				</p>
		<?php endif; ?>
		<p id="submit-buton" style="display: flex; justify-content: center;">
					<label>&nbsp;</label> <input type="submit" name="Submit"
						style="border-radius: 5px;" class="button login"
						value="<?php echo $JText_('JLOGIN'); ?>" />
				</p>
				<input type="hidden" name="option" value="com_users" /> <input
					type="hidden" name="task" value="user.login" /> <input
					type="hidden" name="return" value="<?= $return ?>" />
