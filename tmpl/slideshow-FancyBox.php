...
$resize = function ($url, $src = '', $width = 0, $height = 0) use ($plg) {
    ...
    // @copy($src,$tmp);
    $img = JFactory::getApplication()->triggerEvent('onSizeImage', [
        $url,
        $width,
        $height,
        'big',
        ...
        'cache',
        664
    ]); // ,$width,$height,'big',true,'cache',664
...
foreach ($elements as $i => $module) {
    ...
    $imageName = explode(DIRECTORY_SEPARATOR, str_replace([
        '/',
        '\\'
    ], DIRECTORY_SEPARATOR, $module->image));
