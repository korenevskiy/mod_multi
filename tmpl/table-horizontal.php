...
if ($self_module_tag)
    echo "<$module_tag class=\"multimodule$moduleclass_sfx id$id\"  >";
...
if ($image_show && $image)
    echo "<img class=\"multiimage\" src=\"$image\" alt=\"$title\" />";
...
if ($param->title_tag) {
    ...
    if ($params->get('title_tag') == 'default') {
        $title_tag = '';
    }
    if ($params->get('title_tag') == 'default' && $module->showtitle) {
        $title_tag = $module->header_tag ?: 'div';
    }
    if ($params->get('title_tag') == 'item') {
    ...
    foreach ($headers as $id => $header) {
        ...
        if ($params->get('items_link') == 'ha') {
            $module_title = "<$title_tag class=\" item_title $module->header_class\"><a href='$module->link' class='$class' title='$module->title' >$module->title</a></$title_tag>";
        }
        if ($params->get('items_link') == 'ah') {
            $module_title = "<a href='$module->link' class='$class item_title' title='$module->title' ><$title_tag class=\"  $module->header_class\">$module->title</$title_tag></a>";
        }
        if ($params->get('items_link') == '0') {
            $module_title = "<$title_tag class=\"$class item_title $module->header_class\" title='$module->title' >$module->title</$title_tag>";
        }
