...
echo "<ul class=\"slider items refineSlide  count$count $moduleclass_sfx \"  itemscope itemtype=\"http://schema.org/ImageGallery\">";// id$id pos$positon 
...
if($params->get('items_image') && $module->image):
    if($params->get('items_image') == 'ai')
        echo $prepare("<a class=\"$class image\"  target=\"_blank\" href=\"$module->link\"  title=\"$module->title\"
            ...
            <img class=\" $module->moduleclass_sfx\" src=\"$module->image\"></a>");   
    if($params->get('items_image') == 'di') 
        echo $prepare("<div class='image'><img  class=\"     $module->moduleclass_sfx\"  data-action=\"zoom\"  src=\"$module->image\"></div>");       
    if($params->get('items_image') == 'i') 
        echo $prepare("<img class=\"image $module->moduleclass_sfx\" src=\"$module->image\">");
endif;
...
//if (empty($script)) {
    ...
    JHtml::script($mod_path . 'modernizr.custom.js');
    JHtml::script($mod_path . "Responsive-jQuery-Slideshow-with-3D-CSS3-Transforms-Refine-Slide/jquery.refineslide.min.js");
    JHtml::stylesheet($mod_path . "Responsive-jQuery-Slideshow-with-3D-CSS3-Transforms-Refine-Slide/refineslide.css"); 
    JHtml::stylesheet($mod_path . "Responsive-jQuery-Slideshow-with-3D-CSS3-Transforms-Refine-Slide/style.css"); 
...
$id      = $params->get('id');
$json_3d_css3_transforms      = $params->get('json_3d_css3_transforms');


//toPrint($json_owlCarousel,'$json_owlCarousel');

//https://www.jqueryscript.net/slideshow/Responsive-jQuery-Slideshow-with-3D-CSS3-Transforms-Refine-Slide.html
 $script = <<< script
 

jQuery( function() {
//        console.clear();
        console.log('jQuery:',jQuery.fn.jquery,' -3d_CSS3_transroms -MultiModule:',{$id}); 
//        return;
        jQuery(".slider.items.refineSlide.id{$id}").refineSlide({ 
            {$json_3d_css3_transforms}
        });  
        return; 
